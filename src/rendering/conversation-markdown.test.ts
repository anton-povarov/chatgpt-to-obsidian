...
describe('renderConversationMarkdown', () => {
  it('renders frontmatter, query callouts, metadata, and nested response headings', () => {
    ...
    expect(markdown).toContain('> [!Question] Build a parser\n>\n> ```ts');
    expect(markdown).toContain('*2026-07-12 14:33 · 44 sec · GPT-5.5 Thinking*');
  ...
  it('uses only the first query line in the Exchange heading', () => {
    const markdown = renderConversationMarkdown(
      {
        ...
        exchanges: [
          {
            queryMarkdown: 'explain this arm64 assembly\nstp x29, x30, [sp, #-16]!\nret',
    ...
    expect(markdown).toContain('# Exchange 1 — explain this arm64 assembly\n\n');
    expect(markdown).toContain(
      '> [!Question] explain this arm64 assembly\n> stp x29, x30, [sp, #-16]!\n> ret',
    );
    expect(markdown).not.toContain('# Exchange 1 — explain this arm64 assembly stp');
  });
});
...
describe('demoteMarkdownHeadings', () => {
  it('does not alter hash-prefixed lines inside fenced code', () => {
    expect(demoteMarkdownHeadings('# Heading\n```md\n# Not a heading\n```')).toBe(
      '## Heading\n```md\n# Not a heading\n```',
    );
  });
});
